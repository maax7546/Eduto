...
| Hilfe-Button (?) / geführte, interaktive Tour / Tutorial | `global/ui/elements/tutorial/tutorial.js` (+ `.css`); `global/ui/ui.js` (baut den ?-Button via `injectUI({showHelp:true})`); das **Modul-JS** liefert die Schritte (`window.Tutorial.define([...])`, je mit `advanceOn`) | `window.Tutorial` (§4). Interaktiv auf der echten Seite (Element wird klickbar, fordert zur Aktion auf, zeigt bei Erledigung ein Häkchen und hebt den Weiter-Pfeil hervor — springt NICHT automatisch weiter), öffnet das Settings-Menü autom.; **neuer Tour-Text → in `lang-de.js`/`lang-en.js`** |
