...
- **`regrouping/`** — Modul: `placevaluechart-regrouping`. Gruppen-UI: `ui-regrouping`. Styles: `visuals-regrouping/`.
...
- **CSS-Klassen (global):** `nb-box`, `nb-card`, `nb-ghost`, `nb-symbol`, `nb-dropzone`/`nb-drop-target`, `nb-option`; `dot-frame` (+ `v-10` (1×10) / `v-20` (2×10), `large`, `mode-minus`, `frozen`); `settings-section`, `settings-cycle-btn`, `settings-row`; `big-menu-btn` (+ Farbe `blue/green/orange/purple/red/teal`), `btn-title`; **Tutorial (`tut-`):** `tut-root` (+ `tut-active` auf `<html>`, `tut-instant`), `tut-svg`/`tut-dim`/`tut-hole`/`tut-ring`/`tut-rings`, `tut-blocker` (Sperr-Streifen), `tut-cheer` (+ `tut-cheer-show` = Häkchen bleibt nach erledigter Aktion stehen), `tut-bubble` (+ `tut-in`), `tut-arrow`, `tut-title`/`tut-text`, `tut-dots`/`tut-dot`, `tut-prev`/`tut-next` (+ `tut-next-done` = letzter Schritt, + `tut-next-ready` = Schritt geschafft → Pfeil pulst)/`tut-close`; Header-?-Button `btn-help`/`btn-help-mark`; **SeatTable (`st-`):** `st-table` (+ `st-selected` = in Auswahl, `st-solo` = einzige Auswahl → Griffe, `st-snapped`, `st-no-chairs` = Stühle ausgeblendet), `st-body`, `st-label` (Tisch-Beschriftung, z. B. „Pult"), `st-chair` (+ `st-chair-<side>`), `st-chair-name` (+ `st-has-name`), `st-handle` (+ `st-handle-rotate`/`st-handle-resize`), `st-add-<side>` (+ `st-add-full` = Stuhl-Limit erreicht), `st-del-table`/`st-del-chair`.
